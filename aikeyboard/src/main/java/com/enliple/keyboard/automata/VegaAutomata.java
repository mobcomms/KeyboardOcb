...
public class VegaAutomata extends Automata {
    ...
    private String mPrefComplete = ""; // '갉' 에 'ㄱ' 이 들어올 경우 '갈ㅋ' 가 되어야 하는데 이때 '갈' 을 임시로 저장하는 변수

    public VegaAutomata()
    {
        KeyboardLogPrint.w("VegaAutomata create mState = 0");
        mState = 0;
        mCompositionString = "";
        mCompleteString = "";
...
//					code = (char)((int) code - lcIndex + newIndex);
    ...
    private int DoState01(char code)
    {
        ...
        {
            char newCode = CombineLastConsonantWithCode(mCompositionString.charAt(0), code); // 현재 입력창에 있는 자음과 넘어온 code(자음)의 조합자음, 갈ㄷ + ㄷ 일 경우 rotateCode ㅌ를 가져온다.
            if (newCode == (char) 0) // 조합자음 값이 없다면 (ex : 기존의 값이 ㅂ 이고 새로 들어온 값이 ㅈ 이라면 이 두 값으로 조합자음이 만들어질 수 없는 경우
            {
                ...
                if ( rotatedCode == (char)0) // 순환값이 아닐경우
                {
            ...
            char rotateCode = RotateConsonantWithCode(mCompositionString.charAt(0), code);
        } else {
...
//            ret = ACTION_UPDATE_COMPLETESTR | ACTION_UPDATE_COMPOSITIONSTR;
    ...
    {
        KeyboardLogPrint.e("vega RotateConsonantWithCode :: lcCode1 :: " + lcCode1 + " , lcCode2 :: " + lcCode2);
        char newCode = (char) 0;
        if ( lcCode1 == 0x3131 && lcCode2 == 0x3131) { // 기존 자음 ㄱ, 새로 들어온 자음 ㄱ 일 경우
            newCode = 0x314B; // ㅋ
        } else if ( lcCode1 == 0x314B && lcCode2 == 0x3131 ) { // ㅋ, ㄱ 일 경우 ㄲ 으로 변경
            newCode = 0x3132;
        } else if ( lcCode1 == 0x3132 && lcCode2 == 0x3131) { // ㄲ, ㄱ 일 경우 ㄱ 으로 변경
            newCode = 0x3131;
        }

        else if ( lcCode1 == 0x3137 && lcCode2 == 0x3137 ) { // ㄷ, ㄷ 일 경우 ㅌ 으로 변경
            newCode = 0x314C;
        } else if ( lcCode1 == 0x314C && lcCode2 == 0x3137 ) { // ㅌ, ㄷ 일 경우 ㄸ 으로 변경
            newCode = 0x3138;
        } else if ( lcCode1 == 0x3138 && lcCode2 == 0x3137 ) { // ㄸ, ㄷ 일 경우 ㄷ 으로 변경
            newCode = 0x3137;
        }

        else if ( lcCode1 == 0x3134 && lcCode2 == 0x3134 ) { // ㄴ, ㄴ 일 경우 ㄹ 으로 변경
            newCode = 0x3139;
        } else if ( lcCode1 == 0x3139 && lcCode2 == 0x3134 ) { // ㄹ, ㄴ 일 경우 ㄴ 으로 변경
            newCode = 0x3134;
        }

        else if ( lcCode1 == 0x3141 && lcCode2 == 0x3141 ) { // ㅁ, ㅁ 일 경우 ㅅ 으로 변경
            newCode = 0x3145;
        } else if ( lcCode1 == 0x3145 && lcCode2 == 0x3141 ) { // ㅅ, ㅁ 일 경우 ㅆ 으로 변경
            newCode = 0x3146;
        } else if ( lcCode1 == 0x3146 && lcCode2 == 0x3141 ) { // ㅆ, ㅁ 일 경우 ㅁ 으로 변경
            newCode = 0x3141;
        }

        else if ( lcCode1 == 0x3142 && lcCode2 == 0x3142 ) { // ㅂ, ㅂ 일 경우 ㅍ 으로 변경
            newCode = 0x314D;
        } else if ( lcCode1 == 0x314D && lcCode2 == 0x3142 ) { // ㅍ, ㅂ 일 경우 ㅃ 으로 변경
            newCode = 0x3143;
        } else if ( lcCode1 == 0x3143 && lcCode2 == 0x3142 ) { // ㅃ, ㅂ 일 경우 ㅂ 으로 변경
            newCode = 0x3142;
        }

        else if ( lcCode1 == 0x3148 && lcCode2 == 0x3148 ) { // ㅈ, ㅈ 일 경우 ㅊ 으로 변경
            newCode = 0x314A;
        } else if ( lcCode1 == 0x314A && lcCode2 == 0x3148 ) { // ㅊ, ㅈ 일 경우 ㅉ 으로 변경
            newCode = 0x3149;
        } else if ( lcCode1 == 0x3149 && lcCode2 == 0x3148 ) { // ㅉ, ㅈ 일 경우 ㅈ 으로 변경
            newCode = 0x3148;
        }

        else if ( lcCode1 == 0x3147 && lcCode2 == 0x3147 ) { // ㅇ, ㅇ 일 경우 ㅎ 으로 변경
            newCode = 0x314E;
        } else if ( lcCode1 == 0x314E && lcCode2 == 0x3147 ) { // ㅎ, ㅇ 일 경우 ㅇ 으로 변경
            newCode = 0x3147;
        }
    ...
    {
        KeyboardLogPrint.d("setInitState vega");
    }
}
