...
public class ChunjiinAutomata extends Automata {
...
//        mKoreanMode = false;
    ...
    public CharModel ChunjiinCombineLastConsonantWithCode(char lcCode1, char lcCode2) {
        ...
        if (lcCode1 == 0x3131) // ㄱ
        {
            ...
            {
                newCode = 0x3133; // ㄳ
            } else if (lcCode2 == 0x3131) // ㄱ
            {
                rotatedCode = 0x314B; // ㅋ
                mode = -1;
                newCode = (char) 0;
            }
        } else if (lcCode1 == 0x314B) // ㅋ
        {
            if (lcCode2 == 0x3131) // ㄱ
            {
        ...
        } else if (lcCode1 == 0x3134) // ㄴ
        {
            if (lcCode2 == 0x3134) // ㄴ
            {
                rotatedCode = 0x3139; // ㄹ
                mode = -1;
                newCode = (char) 0;
            } else if (lcCode2 == 0x3148) // ㅈ
                newCode = 0x3135; // ㄵ
        ...
        } else if (lcCode1 == 0x3149) //ㅉ
        {
            if (lcCode2 == 0x3148) // ㅈ
            {
                rotatedCode = 0x3148; // ㅈ
                mode = -1;
                newCode = (char) 0;
            }
        } else if (lcCode1 == 0x3147) // ㅇ   ...
        {
            if (lcCode2 == 0x3147) // ㅇ
            {
                rotatedCode = 0x3141; // ㅁ
                mode = -1;
                newCode = (char) 0;
            }
        } else if (lcCode1 == 0x3141) // ㅁ
        {
            if (lcCode2 == 0x3147) // ㅇ
            {
                rotatedCode = 0x3147; // ㅇ
                mode = -1;
                newCode = (char) 0;
            }
    ...
    private int DoState01(char code) {
        ...
        if (IsConsonant(code)) // 입력 코드가 자음일 경우
        {
            ...
            {
                if (rotatedCode == (char) 0) // 조합 자음 없음, 순환값이 아닐 경우
                {
                ...
                else // 순환값일 경우
                {
                    ...
                    if ("".equals(mCompleteString)) { // 조합자음이 없는 상태에서 순환값, mCompleteString 없음
                        if (!"".equals(mPrefComplete)) { // 조합자음이 없는 상태에서 순환값, mCompleteString 없음, mPrefComplete는 갍 -> 갈ㄷ으로 갈때 '갈'을 임시로 저장하는 변수인데 이 값이 있을 경우
                            ...
                            if (cConsonant != (char) 0) { // 조합자음이 없는 상태에서 순환값, mCompleteString 없음, mPrefComplete 있음.
                    ...
                    } else { // 조합자음이 없는 상태에서 순환값, mCompleteString 있음
                        int completeLastConsonant = GetLastConsonantIndex(mCompleteString.charAt(0)); // 완성된 앞글자의 종성자음
                        char lastConsonant = GetLastConsonant(completeLastConsonant);// 완성된 앞글자의 종성자음(char)
                        KeyboardLogPrint.d("lastConsonant :: " + lastConsonant);
                        KeyboardLogPrint.i("completeLastConsonant : " + completeLastConsonant);
                        // rotatedCode 순환되어진 값.
                        CharModel combinedLastConsonanat = ChunjiinCombineLastConsonantWithCode(lastConsonant, rotatedCode);
                        char cConsonant = combinedLastConsonanat.getChar();
                        KeyboardLogPrint.i("cConsonant : " + cConsonant);
                        if (cConsonant != (char) 0) { // 조합자음이 없는 상태에서 순환값, mCompleteString 있음, mCompleteString의 종성자음과 순환코드로 값이 만들어졌다면
                            KeyboardLogPrint.w("DoState01 :: 6");
                            int iVowel = GetVowelIndex(mCompleteString.charAt(0));
                            KeyboardLogPrint.d("iVowel :: " + iVowel);
                            if ( iVowel >= 0 ) {
                                KeyboardLogPrint.w("DoState01 :: 6-1");
                                int lastConsonantIndex = GetLastConsonantIndex(cConsonant);
                                ...
                                KeyboardLogPrint.w("DoState01 :: 6-2");
...
//                        mStateChangeTime1 = curTime;
                    } else // '같' 다음 ㄷ이 올 경우 ㄸ은 종성자음으로 쓸 수 없음
                    {
                        ...
                        KeyboardLogPrint.d("chunjiin DoState03 lastConsonant :: " + lastConsonant);
...
//    }
    ...
    public void setInitState() {
        KeyboardLogPrint.w("setInitState chunjiin : " + mState + " : " + mCompleteString + " : " + mCompositionString + " : " + mPrefComplete);
        mState = 0;
        mPrefComplete = "";
        mCompleteString = "";
        mCompositionString = "";
