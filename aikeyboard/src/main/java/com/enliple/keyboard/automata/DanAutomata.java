...
public class DanAutomata extends Automata {
...
//	}
    ...
    public char RotatedVowelWithCode(char vCode1, char vCode2)
    {
        char newCode = (char) 0;
        if ( vCode1 == vCode2 )
        {
            ...
            else if ( vCode1 == 0x3150 ) // ㅐ
            {
                newCode = 0x3152; // ㅒ
            }
            else if ( vCode1 == 0x3154 ) // ㅔ
            {
                newCode = 0x3156; // ㅖ
            }
    ...
    public char RotateConsonantWithCode(char lcCode1, char lcCode2)
    {
        char newCode = (char) 0;
        if ( lcCode1 == lcCode2 )
        {
            if ( lcCode1 == 0x3142 ){
                // ㅂ
                KeyboardLogPrint.e("dan lcCode1 ㅂ");
                newCode = 0x3143; // ㅃ
            } else if ( lcCode1 == 0x3148 ) // ㅈ
                newCode = 0x3149; // ㅉ
            else if ( lcCode1 == 0x3137) // ㄷ
            {
                KeyboardLogPrint.e("dan lcCode1 ㄷ");
                newCode = 0x3138; // ㄸ
            }
            else if ( lcCode1 == 0x3131 ) // ㄱ
                newCode = 0x3132; //ㄲ
            else if ( lcCode1 == 0x3145) // ㅅ
                newCode = 0x3146; // ㅆ
        }
        return newCode;
    }
...
//					code = (char)((int) code - lcIndex + newIndex);
    ...
    private int DoState01(char code)
    {
        ...
        {
            char rotatedCode = RotateConsonantWithCode(mCompositionString.charAt(0), code);
            if ( rotatedCode != (char)0 )
            {
                KeyboardLogPrint.e("DoState01 rotatedCode :: " + rotatedCode);
                mCompositionString = "";
...
//                ret = ACTION_REMOVE_PREV_CHAR | ACTION_UPDATE_COMPLETESTR | ACTION_UPDATE_COMPOSITIONSTR;
            ...
            {
                KeyboardLogPrint.e("DoState01 rotatedCode does not exist");
...
//            }
    ...
    private int DoState03(char code)
    {
        ...
        if (IsConsonant(code))
        {
            KeyboardLogPrint.d("DoSate03 자음");
            int lcIndex = GetLastConsonantIndex(mCompositionString.charAt(0));
            char lcChar = InputTables.LastConsonants.Code[lcIndex];
            KeyboardLogPrint.e("dan lcChar :: " + + lcChar);
            KeyboardLogPrint.e("dan code :: " + + code);
            char rotatedChar = RotateConsonantWithCode(lcChar, code);

            if ( rotatedChar != (char)0 )
            {
                KeyboardLogPrint.d("DoSate03 rotatedChar not zero. rotatedChar :: " + rotatedChar);
                int rotatedConsonantIndex = GetLastConsonantIndex(rotatedChar);
                KeyboardLogPrint.d("DoSate03 rotatedConsonantIndex :: " + rotatedConsonantIndex);
                if ( rotatedConsonantIndex != -1 )
                {
                    char newChar = (char) ((int) mCompositionString.charAt(0) - lcIndex + rotatedConsonantIndex);
                    mCompleteString = "";
                    mCompositionString = "";
...
//                    mState = 1;
//                    ret = ACTION_UPDATE_COMPLETESTR | ACTION_UPDATE_COMPOSITIONSTR;
                    // 20230918 갑 + ㅂ 일 때 가ㅃ 가 만들어지지 않는 현상 수정
                    int lastConsonant = GetLastConsonantIndex(mCompositionString.charAt(0));
                    KeyboardLogPrint.d("dan DoState03 lastConsonant :: " + lastConsonant);
                    char newChar = (char) ((int) mCompositionString.charAt(0) - lastConsonant);
                    mCompleteString = "";
                    mCompositionString = "";
                    mCompleteString += newChar; // 기존 갑이었던 것을 '가'로 바꿔주고
                    mCompositionString += rotatedChar; // '가' 뒤에 ㅃ 를 붙여서 '갑' 이후 ㅂ이 들어왔을 때 가ㅃ 로 만들어준다.
                    mState = 1;
                    KeyboardLogPrint.w("dan DoState03 3 mCompleteString :: " + mCompleteString);
                    ret = ACTION_UPDATE_COMPLETESTR | ACTION_UPDATE_COMPOSITIONSTR;
                }
            }
            else
...
//                int lcIndex = GetLastConsonantIndex(mCompositionString.charAt(0));
    ...
    private int DoState04(char code) // current composition string: single vowel
    {
        ...
        {
            char rotatedVowel = RotatedVowelWithCode(mCompositionString.charAt(0), code);
            if ( rotatedVowel != (char)0 )
            {
                mCompositionString = "";
    ...
    {
        KeyboardLogPrint.d("DoState21 :: " + code);
        KeyboardLogPrint.i("DoState21 mCompositionString : " + mCompositionString);
        KeyboardLogPrint.i("DoState21 mCompleteString : " + mCompleteString);
        if (mCompositionString == "")
        {
            return ACTION_ERROR;
        }
        ...
        {
            int lcIndex = GetLastConsonantIndex(mCompositionString.charAt(0));
            char lcCode = InputTables.LastConsonants.Code[lcIndex];

            char rotatedConsonant = RotateConsonantWithCode(lcCode, code);

            if ( rotatedConsonant != (char) 0 )
            {
                int rotatedIndex = GetLastConsonantIndex(rotatedConsonant);
                char newChar = mCompositionString.charAt(0);
                newChar = (char)((int)newChar - lcIndex + rotatedIndex);
                mCompleteString = "";
                mCompositionString = "";
                mCompositionString += newChar;
                mState = 21;
    ...
    {
        KeyboardLogPrint.d("setInitState dan");
        mState = 0;
        mCompleteString = "";
        mCompositionString = "";
    }
