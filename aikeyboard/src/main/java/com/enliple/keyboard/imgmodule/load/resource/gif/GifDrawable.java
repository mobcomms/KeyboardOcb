...
import static com.enliple.keyboard.imgmodule.gifdecoder.GifDecoder.TOTAL_ITERATION_COUNT_FOREVER;
...
import com.enliple.keyboard.imgmodule.load.resource.Animatable2Compat;
...
public class GifDrawable extends Drawable
   ...
   * @param frameTransformation An {@link com.enliple.keyboard.imgmodule.load.Transformation} that can be
   *     applied to each frame.
   * @param targetFrameWidth The desired width of the frames displayed by this drawable (the width
   *     of the view or {@link com.enliple.keyboard.imgmodule.request.target.Target} this drawable is being
   *     loaded into).
   * @param targetFrameHeight The desired height of the frames displayed by this drawable (the
   *     height of the view or {@link com.enliple.keyboard.imgmodule.request.target.Target} this drawable is
   *     being loaded into).
   * @param gifDecoder The decoder to use to decode GIF data.
   * @param firstFrame The decoded and transformed first frame of this GIF.
   * @see #setFrameTransformation(com.enliple.keyboard.imgmodule.load.Transformation, android.graphics.Bitmap)
    ...
    if (loopCount <= 0 && loopCount != LOOP_FOREVER && loopCount != LOOP_INTRINSIC) {
      throw new IllegalArgumentException(
          "Loop count must be greater than 0, or equal to "
              + "ImageDrawable.LOOP_FOREVER, or equal to ImageDrawable.LOOP_INTRINSIC");
    }
