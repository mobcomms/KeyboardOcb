...
import com.enliple.keyboard.imgmodule.ImageBuilder.LogRequestOrigins;
...
import com.enliple.keyboard.imgmodule.load.resource.drawable.DrawableDecoderCompat;
import com.enliple.keyboard.imgmodule.request.target.SizeReadyCallback;
...
import com.enliple.keyboard.imgmodule.request.transition.TransitionFactory;
import com.enliple.keyboard.imgmodule.util.LogTime;
import com.enliple.keyboard.imgmodule.util.Util;
import com.enliple.keyboard.imgmodule.util.pool.StateVerifier;
import java.util.List;
import java.util.concurrent.Executor;

/**
 * A {@link Request} that loads a {@link com.enliple.keyboard.imgmodule.load.engine.Resource} into a given
 * {@link Target}.
 *
...
public final class SingleRequest<R> implements Request, SizeReadyCallback, ResourceCallback {
  ...
  private static final String IMAGEMODULE_TAG = "ImageModule";
  ...
  private SingleRequest(
    ...
    if (requestOrigin == null && imageContext.getExperiments().isEnabled(LogRequestOrigins.class)) {
      requestOrigin = new RuntimeException("ImageModule request origin trace");
    }
  }
  ...
  public void begin() {
    synchronized (requestLock) {
      ...
      if (model == null) {
        ...
        onLoadFailed(new ImageModuleException("Received null model"), logLevel);
        return;
      }
  ...
  private Drawable loadDrawable(@DrawableRes int resourceId) {
    ...
    return DrawableDecoderCompat.getDrawable(imageContext, resourceId, theme);
  }
  ...
  public void onResourceReady(
    ...
    try {
      synchronized (requestLock) {
        loadStatus = null;
        if (resource == null) {
          ImageModuleException exception =
              new ImageModuleException(
  ...
  private void onResourceReady(
    ...
    if (imageContext.getLogLevel() <= Log.DEBUG) {
      Log.d(
              IMAGEMODULE_TAG,
          "Finished loading "
  ...
  /** A callback method that should never be invoked directly. */
  @Override
  public void onLoadFailed(ImageModuleException e) {
  ...
  private void onLoadFailed(ImageModuleException e, int maxLogLevel) {
    stateVerifier.throwIfRecycled();
    synchronized (requestLock) {
      e.setOrigin(requestOrigin);
      int logLevel = imageContext.getLogLevel();
      if (logLevel <= maxLogLevel) {
        Log.w(
                IMAGEMODULE_TAG, "Load failed for " + model + " with size [" + width + "x" + height + "]", e);
        if (logLevel <= Log.INFO) {
          e.logRootCauses(IMAGEMODULE_TAG);
        }
      }
