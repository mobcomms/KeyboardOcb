...
import com.enliple.keyboard.imgmodule.load.model.Headers;
...
public abstract class BaseImageUrlLoader<Model> implements ModelLoader<Model, InputStream> {
  private final ModelLoader<ImageUrl, InputStream> concreteLoader;
  @Nullable private final ModelCache<Model, ImageUrl> modelCache;

  protected BaseImageUrlLoader(ModelLoader<ImageUrl, InputStream> concreteLoader) {
    this(concreteLoader, null);
  }

  protected BaseImageUrlLoader(
      ModelLoader<ImageUrl, InputStream> concreteLoader,
      @Nullable ModelCache<Model, ImageUrl> modelCache) {
  ...
  public LoadData<InputStream> buildLoadData(
    ...
    if (result == null) {
      ...
      result = new ImageUrl(stringURL, getHeaders(model, width, height, options));
  ...
  private static List<Key> getAlternateKeys(Collection<String> alternateUrls) {
    ...
    for (String alternate : alternateUrls) {
      result.add(new ImageUrl(alternate));
    }
    return result;
  }
   ...
   * as the primary url.
   *
   * <p>Implementing this method allows ImageModule to fulfill requests for bucketed images in smaller
