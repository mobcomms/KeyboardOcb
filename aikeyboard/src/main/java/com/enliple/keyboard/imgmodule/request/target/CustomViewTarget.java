...
public abstract class CustomViewTarget<T extends View, Z> implements Target<Z> {
  ...
  @IdRes private static final int VIEW_TAG_ID = R.id.imagemodule_custom_view_target_tag;
   ...
   * uses {@link View#setTag(Object)}. It is only necessary if loading several ImageModule resources into
   * the same view, for example one foreground and one background view.
   *
   * @param tagId The android resource id to use.
   * @deprecated Using this method prevents clearing the target from working properly. ImageModule uses
    ...
    private int getTargetDimen(int viewSize, int paramSize, int paddingSize) {
      ...
      if (!view.isLayoutRequested() && paramSize == LayoutParams.WRAP_CONTENT) {
        if (Log.isLoggable(TAG, Log.INFO)) {
          Log.i(
              TAG,
              "treats LayoutParams.WRAP_CONTENT as a request for an image the size of"
