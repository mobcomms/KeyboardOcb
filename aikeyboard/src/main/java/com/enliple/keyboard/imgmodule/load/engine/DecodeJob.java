...
import com.enliple.keyboard.imgmodule.load.data.DataFetcher;
...
import com.enliple.keyboard.imgmodule.util.pool.FactoryPools.Poolable;
import com.enliple.keyboard.imgmodule.util.pool.ImageModuleTrace;
...
class DecodeJob<R>
  ...
  public void run() {
    ...
    ImageModuleTrace.beginSectionFormat("DecodeJob#run(model=%s)", model);
  ...
  private void notifyFailed() {
    setNotifiedOrThrow();
    ImageModuleException e = new ImageModuleException("Failed to load resource", new ArrayList<>(throwables));
  ...
  public void onDataFetcherFailed(
    ...
    fetcher.cleanup();
    ImageModuleException exception = new ImageModuleException("Fetching data failed", e);
  ...
  private <Data> Resource<R> decodeFromData(
      DataFetcher<?> fetcher, Data data, DataSource dataSource) throws ImageModuleException {
    try {
      if (data == null) {
        return null;
      }
  ...
  private <Data, ResourceType> Resource<R> runLoadPath(
    ...
    DataRewinder<Data> rewinder = imageContext.getRegistry().getRewinder(data);
    try {
  ...
  <Z> Resource<Z> onResourceDecoded(DataSource dataSource, @NonNull Resource<Z> decoded) {
    ...
    if (dataSource != DataSource.RESOURCE_DISK_CACHE) {
      ...
      transformed = appliedTransformation.transform(imageContext, decoded, width, height);
    }
