...
/** An exception with zero or more causes indicating why a load in ImageModule failed. */
// Public API.
@SuppressWarnings("WeakerAccess")
public final class ImageModuleException extends Exception {
  ...
  public ImageModuleException(String message) {
    this(message, Collections.<Throwable>emptyList());
  }

  public ImageModuleException(String detailMessage, Throwable cause) {
    this(detailMessage, Collections.singletonList(cause));
  }

  public ImageModuleException(String detailMessage, List<Throwable> causes) {
   ...
   *
   * <p>Causes may or may not be {@link ImageModuleException ImageModuleExceptions}. Causes may also not be root
   ...
   * failed due to an error that can be retried. Keep in mind that because ImageModule may attempt to load
  ...
  private void addRootCauses(Throwable throwable, List<Throwable> rootCauses) {
    if (throwable instanceof ImageModuleException) {
      ImageModuleException imageModuleException = (ImageModuleException) throwable;
      for (Throwable t : imageModuleException.getCauses()) {
  ...
  public String getMessage() {
    ...
    result.append("\n call ImageModuleException#logRootCauses(String) for more detail");
    return result.toString();
  }
  ...
  private static void appendCausesWrapped(List<Throwable> causes, Appendable appendable)
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (cause instanceof ImageModuleException) {
        ImageModuleException imageModuleCause = (ImageModuleException) cause;
        imageModuleCause.printStackTrace(appendable);
      } else {
