...
/**
 * Generates a series of {@link com.enliple.keyboard.imgmodule.load.data.DataFetcher DataFetchers} using
 * registered {@link com.enliple.keyboard.imgmodule.load.model.ModelLoader ModelLoaders} and a model.
 */
interface DataFetcherGenerator {
  /**
   * Called when the generator has finished loading data from a {@link
   * com.enliple.keyboard.imgmodule.load.data.DataFetcher}.
   */
  interface FetcherReadyCallback {

    /** Requests that we call startNext() again on a ImageModule owned thread. */
    void reschedule();
  ...
  /**
   * Attempts to a single new {@link com.enliple.keyboard.imgmodule.load.data.DataFetcher} and returns true if a
   * {@link com.enliple.keyboard.imgmodule.load.data.DataFetcher} was started, and false otherwise.
   */
  boolean startNext();
