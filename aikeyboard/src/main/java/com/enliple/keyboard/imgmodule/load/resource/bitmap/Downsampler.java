...
import com.enliple.keyboard.imgmodule.load.ImageHeaderParser.ImageType;
import com.enliple.keyboard.imgmodule.load.Option;
import com.enliple.keyboard.imgmodule.load.Options;
import com.enliple.keyboard.imgmodule.load.PreferredColorSpace;
...
import com.enliple.keyboard.imgmodule.load.resource.bitmap.DownsampleStrategy.SampleSizeRounding;
import com.enliple.keyboard.imgmodule.request.RequestOptions;
import com.enliple.keyboard.imgmodule.request.target.Target;
import com.enliple.keyboard.imgmodule.util.LogTime;
...
public final class Downsampler {
  ...
  public static final Option<DecodeFormat> DECODE_FORMAT =
      Option.memory(
          "com.enliple.keyboard.imgmodule.load.resource.bitmap.Downsampler.DecodeFormat", DecodeFormat.DEFAULT);
  ...
  public static final Option<PreferredColorSpace> PREFERRED_COLOR_SPACE =
      Option.memory(
          "com.enliple.keyboard.imgmodule.load.resource.bitmap.Downsampler.PreferredColorSpace",
          PreferredColorSpace.SRGB);
  /**
   * Indicates the {@link com.enliple.keyboard.imgmodule.load.resource.bitmap.DownsampleStrategy} option that
  ...
  public static final Option<Boolean> FIX_BITMAP_SIZE_TO_REQUESTED_DIMENSIONS =
      Option.memory("com.enliple.keyboard.imgmodule.load.resource.bitmap.Downsampler.FixBitmapSize", false);

  /**
   * Indicates that it's safe or unsafe to decode {@link Bitmap}s with {@link
   * Bitmap.Config#HARDWARE}.
   *
   * <p>Callers should almost never set this value to {@code true} manually. ImageModule will already do
   * so when ImageModule believes it's safe to do (when no transformations are applied). Instead, callers
   * can set this value to {@code false} to prevent ImageModule from decoding hardware bitmaps if ImageModule is
  ...
  public static final Option<Boolean> ALLOW_HARDWARE_CONFIG =
      Option.memory(
          "com.enliple.keyboard.imgmodule.load.resource.bitmap.Downsampler.AllowHardwareDecode", false);
   ...
   * provided {@link com.enliple.keyboard.imgmodule.load.resource.bitmap.DownsampleStrategy} option.
   *
   ...
   * com.enliple.keyboard.imgmodule.load.engine.bitmap_recycle.BitmapPool} whose dimensions exactly match those
