...
/**
 * ImageModule supported handling of color spaces on Android O+, defaults to {@link #SRGB}.
 *
 * <p>On Android O, ImageModule will always request SRGB and will ignore this option if set. A bug on
 * Android O prevents P3 images from being compressed correctly and can result in color distortion.
 * We may eventually work around this in ImageModule if sufficient demand arises, but doing so will
 * require a memory intensive conversion to SRGB prior to compressing Bitmaps to ImageModule disk cache.
 * This work around would also work only for ImageModule compression, not for any compression that a
 * caller performs on a Bitmap returned by ImageModule.
 *
 * <p>On Android P+, ImageModule supports SRGB and display P3. However, if display p3 is requested, we
 ...
 * Currently ImageModule does not attempt to detect whether or not this support is present. Do not use
 ...
 * quality and washed out looking images. When in doubt, always use {@link #SRGB}, which is ImageModule
 * default.
 *
