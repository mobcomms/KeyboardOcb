package com.enliple.keyboard.imgmodule.request.target;
 ...
 * View#setTag(Object)} on a view, use {@link #setTagId(int)} to specify a custom tag for ImageModule to
 * use.
 *
...
public abstract class ViewTarget<T extends View, Z> extends BaseTarget<Z> {
  ...
  private static int tagId = R.id.imagemodule_custom_view_target_tag;
  ...
  /**
   * @param waitForLayout If set to {@code true}, ImageModule will always wait for any pending layout pass
   *     before checking for the size a View. If set to {@code false} ImageModule will only wait for a
   *     pending layout pass if it's unable to resolve the size from layout parameters or an
   *     existing View size. Because setting this parameter to {@code true} forces ImageModule to wait for
   ...
   *
   * <p>Using this method can save memory by allowing ImageModule to more eagerly clear resources when
  ...
  /**
   * Indicates that ImageModule should always wait for any pending layout pass before checking for the
   * size an {@link View}.
   *
   * <p>By default, ImageModule will only wait for a pending layout pass if it's unable to resolve the
   * size from the {@link LayoutParams} or valid non-zero values for {@link View#getWidth()} and
   * {@link View#getHeight()}.
   *
   * <p>Because calling this method forces ImageModule to wait for the layout pass to occur before
   * starting loads, setting this parameter to {@code true} can cause ImageModule to asynchronous load an
   * image even if it's in the memory cache. The load will happen asynchronously because ImageModule has
   ...
   *
   * <p>For ImageModule to function correctly, ImageModule must be the only thing that calls {@link
   * View#setTag(Object)}. If the tag is cleared or put to another object type, ImageModule will not be
   * able to retrieve and cancel previous loads which will not only prevent ImageModule from reusing
   ...
   * com.enliple.keyboard.imgmodule.request.Request}.
   */
  @Override
  @Nullable
    ...
    if (tag != null) {
      if (tag instanceof Request) {
        ...
        throw new IllegalArgumentException(
            "You must not call setTag() on a view ImageModule is targeting");
      }
    }
    return request;
  }
   ...
   * scrolling lists.
   *
   * <p>If no tag id is set, ImageModule will use {@link View#setTag(Object)}.
   *
   ...
   *
   * @deprecated ImageModule uses it's own default tag id, so there's no need to specify your own. This
    ...
    private int getTargetDimen(int viewSize, int paramSize, int paddingSize) {
      ...
      if (!view.isLayoutRequested() && paramSize == LayoutParams.WRAP_CONTENT) {
        if (Log.isLoggable(TAG, Log.INFO)) {
          Log.i(
              TAG,
              "LayoutParams.WRAP_CONTENT as a request for an image the size of this"
