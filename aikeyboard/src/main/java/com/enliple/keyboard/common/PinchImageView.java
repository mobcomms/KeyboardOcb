...
public class PinchImageView extends ImageView {
    ...
    public Matrix getInnerMatrix(Matrix matrix) {
        ...
        if (isReady()) {
            ...
            if (fillScale == 0)
                fillScale = Math.max((float) getHeight() / (float) getDrawable().getIntrinsicHeight(), (float) getWidth() / (float) getDrawable().getIntrinsicWidth());
    ...
    public void setCenterPosition()
    {
        ...
        if ( getWidth() == bound.width() )
        {
            float scrollValY = (bound.height() - getHeight()) / 2;
            scrollBy(0f, -scrollValY);
        }
        else if ( getHeight() == bound.height() )
        {
            float scrollValX = (bound.width() - getWidth()) / 2;
            scrollBy(-scrollValX, 0f);
        }
        else
        {
            KeyboardLogPrint.w("scroll 할 필요 없음");
        }
    }
    ...
    public void setBgAlpha(int _alpha) {
