...
import com.enliple.httpmodule.internal.tls.CertificateChainCleaner;
import com.enliple.httpmodule.internal.tls.OkHostnameVerifier;
import com.enliple.httpmodule.internal.ws.RealWebSocket;
...
import static com.enliple.httpmodule.internal.Util.assertionError;
...
public class MobonOkHttpClient implements Cloneable, Call.Factory, WebSocket.Factory {
  ...
  static {
    Internal.instance = new Internal() {
      ...
      @Override
      public int code(MobonResponse.Builder responseBuilder) {
      ...
      @Override
      public Call newWebSocketCall(MobonOkHttpClient client, MobonRequest originalMobonRequest) {
        return RealCall.newRealCall(client, originalMobonRequest, true);
      }
    };
  }
  ...
  /**
   * Returns an immutable list of interceptors that observe a single network mobonRequest and response.
   * These interceptors must call {@link Interceptor.Chain#proceed} exactly once: it is an error for
   * a network interceptor to short-circuit or repeat a network mobonRequest.
   */
  ...
  /**
   * Prepares the {@code mobonRequest} to be executed at some point in the future.
   */
  @Override
  public Call newCall(Context context, MobonRequest mobonRequest) {
    return RealCall.newRealCall(context,this, mobonRequest, false /* for web socket */);
  }

  @Override
  public Call newCall(MobonRequest mobonRequest) {
    return RealCall.newRealCall(this, mobonRequest, false /* for web socket */);
  }

  /**
   * Uses {@code mobonRequest} to connect a new web socket.
   */
  @Override
  public WebSocket newWebSocket(MobonRequest mobonRequest, WebSocketListener listener) {
    RealWebSocket webSocket = new RealWebSocket(mobonRequest, listener, new Random(), pingInterval);
  ...
  public static final class Builder {
    ...
    Builder(MobonOkHttpClient mobonOkHttpClient) {
      this.dispatcher = mobonOkHttpClient.dispatcher;
      this.proxy = mobonOkHttpClient.proxy;
      this.protocols = mobonOkHttpClient.protocols;
      this.connectionSpecs = mobonOkHttpClient.connectionSpecs;
      this.interceptors.addAll(mobonOkHttpClient.interceptors);
      this.networkInterceptors.addAll(mobonOkHttpClient.networkInterceptors);
      this.eventListenerFactory = mobonOkHttpClient.eventListenerFactory;
      this.proxySelector = mobonOkHttpClient.proxySelector;
      this.cookieJar = mobonOkHttpClient.cookieJar;
      this.internalCache = mobonOkHttpClient.internalCache;
      this.cache = mobonOkHttpClient.cache;
      this.socketFactory = mobonOkHttpClient.socketFactory;
      this.sslSocketFactory = mobonOkHttpClient.sslSocketFactory;
      this.certificateChainCleaner = mobonOkHttpClient.certificateChainCleaner;
      this.hostnameVerifier = mobonOkHttpClient.hostnameVerifier;
      this.certificatePinner = mobonOkHttpClient.certificatePinner;
      this.proxyAuthenticator = mobonOkHttpClient.proxyAuthenticator;
      this.authenticator = mobonOkHttpClient.authenticator;
      this.connectionPool = mobonOkHttpClient.connectionPool;
      this.dns = mobonOkHttpClient.dns;
      this.followSslRedirects = mobonOkHttpClient.followSslRedirects;
      this.followRedirects = mobonOkHttpClient.followRedirects;
      this.retryOnConnectionFailure = mobonOkHttpClient.retryOnConnectionFailure;
      this.connectTimeout = mobonOkHttpClient.connectTimeout;
      this.readTimeout = mobonOkHttpClient.readTimeout;
      this.writeTimeout = mobonOkHttpClient.writeTimeout;
      this.pingInterval = mobonOkHttpClient.pingInterval;
    }
     ...
     * including on {@link Source} of the {@link MobonResponse}. The default value is 10 seconds.
     *
     ...
     *
     *   MobonOkHttpClient client = new MobonOkHttpClient.Builder()
     ...
     *       failure to reach any individual IP address doesn't fail the overall mobonRequest. This can
     *       increase availability of multi-homed services.
     *   <li><strong>Stale pooled connections.</strong> The {@link ConnectionPool} reuses sockets
     *       to decrease mobonRequest latency, but these connections will occasionally time out.
    ...
    /**
     * Returns a modifiable list of interceptors that observe a single network mobonRequest and response.
     * These interceptors must call {@link Interceptor.Chain#proceed} exactly once: it is an error
     * for a network interceptor to short-circuit or repeat a network mobonRequest.
     */
    public List<Interceptor> networkInterceptors() {
      return networkInterceptors;
    }
