 ...
 */
package com.enliple.httpmodule.internal.ws;
...
import com.enliple.httpmodule.WebSocket;
import com.enliple.httpmodule.WebSocketListener;
...
public final class RealWebSocket implements WebSocket, WebSocketReader.FrameCallback {
  ...
  private final MobonRequest originalMobonRequest;
  ...
  public RealWebSocket(MobonRequest mobonRequest, WebSocketListener listener, Random random,
                       long pingIntervalMillis) {
    if (!"GET".equals(mobonRequest.method())) {
      throw new IllegalArgumentException("MobonRequest must be GET: " + mobonRequest.method());
    }
    this.originalMobonRequest = mobonRequest;
  ...
  public void connect(MobonOkHttpClient client) {
    client = client.newBuilder()
        .eventListener(EventListener.NONE)
        .protocols(ONLY_HTTP1)
        .build();
    final MobonRequest mobonRequest = originalMobonRequest.newBuilder()
        .header("Upgrade", "websocket")
        .header("Connection", "Upgrade")
        .header("Sec-WebSocket-Key", key)
        .header("Sec-WebSocket-Version", "13")
        .build();
    call = Internal.instance.newWebSocketCall(client, mobonRequest);
    call.enqueue(new Callback() {
      @Override
      public void onResponse(Call call, MobonResponse mobonResponse) {
        try {
          checkResponse(mobonResponse);
        } catch (ProtocolException e) {
          failWebSocket(e, mobonResponse);
          closeQuietly(mobonResponse);
          return;
        }
        ...
        try {
          listener.onOpen(RealWebSocket.this, mobonResponse);
          String name = "OkHttp WebSocket " + mobonRequest.url().redact();
  ...
  void checkResponse(MobonResponse mobonResponse) throws ProtocolException {
    if (mobonResponse.code() != 101) {
      throw new ProtocolException("Expected HTTP 101 mobonResponse but was '"
          + mobonResponse.code() + " " + mobonResponse.message() + "'");
    }

    String headerConnection = mobonResponse.header("Connection");
    if (!"Upgrade".equalsIgnoreCase(headerConnection)) {
      throw new ProtocolException("Expected 'Connection' header value 'Upgrade' but was '"
          + headerConnection + "'");
    }

    String headerUpgrade = mobonResponse.header("Upgrade");
    if (!"websocket".equalsIgnoreCase(headerUpgrade)) {
      throw new ProtocolException(
          "Expected 'Upgrade' header value 'websocket' but was '" + headerUpgrade + "'");
    }

    String headerAccept = mobonResponse.header("Sec-WebSocket-Accept");
  ...
  boolean writeOneFrame() throws IOException {
    ...
    try {
      if (pong != null) {
        ...
        BufferedSink sink = MobonOkio.buffer(writer.newMessageSink(
  ...
  public void failWebSocket(Exception e, @Nullable MobonResponse mobonResponse) {
    ...
    try {
      listener.onFailure(this, e, mobonResponse);
    } finally {
      closeQuietly(streamsToClose);
    }
  }
