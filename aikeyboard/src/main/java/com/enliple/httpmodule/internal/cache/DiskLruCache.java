...
public final class DiskLruCache implements Closeable, Flushable {
    ...
    public void run() {
      synchronized (DiskLruCache.this) {
        ...
        try {
          ...
          journalWriter = MobonOkio.buffer(MobonOkio.blackhole());
        }
      }
    }
  };
    ...
    if (fileSystem.exists(journalFile)) {
      try {
        ...
        Platform.get().log(Platform.WARN, "DiskLruCache " + directory + " is corrupt: "
            + journalIsCorrupt.toString() + ", removing", journalIsCorrupt);
      }
  ...
  private void readJournal() throws IOException {
    BufferedSource source = MobonOkio.buffer(fileSystem.source(journalFile));
    try {
  ...
  private BufferedSink newJournalWriter() throws FileNotFoundException {
    ...
    return MobonOkio.buffer(faultHidingSink);
  }
  ...
  synchronized void rebuildJournal() throws IOException {
    ...
    BufferedSink writer = MobonOkio.buffer(fileSystem.sink(journalFileTmp));
    try {
