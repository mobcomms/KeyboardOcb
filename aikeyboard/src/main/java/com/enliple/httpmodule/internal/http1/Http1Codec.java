 ...
 */
package com.enliple.httpmodule.internal.http1;
...
import com.enliple.httpmodule.ForwardingTimeout;
...
import static com.enliple.httpmodule.internal.Util.checkOffsetAndCount;
...
public final class Http1Codec implements HttpCodec {
  ...
  public Http1Codec(MobonOkHttpClient client, StreamAllocation streamAllocation, BufferedSource source,
                    BufferedSink sink) {
    this.client = client;
  ...
  public Sink createRequestBody(MobonRequest mobonRequest, long contentLength) {
    if ("chunked".equalsIgnoreCase(mobonRequest.header("Transfer-Encoding"))) {
      // Stream a mobonRequest body of unknown length.
      return newChunkedSink();
    }

    if (contentLength != -1) {
      // Stream a mobonRequest body of a known length.
      return newFixedLengthSink(contentLength);
    }

    throw new IllegalStateException(
        "Cannot stream a mobonRequest body without chunked encoding or a known content length!");
  }
  ...
  public void writeRequestHeaders(MobonRequest mobonRequest) throws IOException {
    String requestLine = RequestLine.get(
            mobonRequest, streamAllocation.connection().route().proxy().type());
    writeRequest(mobonRequest.headers(), requestLine);
  }
  ...
  public ResponseBody openResponseBody(MobonResponse mobonResponse) throws IOException {
    ...
    if (!HttpHeaders.hasBody(mobonResponse)) {
      ...
      return new RealResponseBody(contentType, 0, MobonOkio.buffer(source));
    }

    if ("chunked".equalsIgnoreCase(mobonResponse.header("Transfer-Encoding"))) {
      Source source = newChunkedSource(mobonResponse.request().url());
      return new RealResponseBody(contentType, -1L, MobonOkio.buffer(source));
    }
    ...
    return new RealResponseBody(contentType, -1L, MobonOkio.buffer(newUnknownLengthSource()));
  }
  ...
  public MobonResponse.Builder readResponseHeaders(boolean expectContinue) throws IOException {
    ...
    try {
      ...
      MobonResponse.Builder responseBuilder = new MobonResponse.Builder()
  ...
  private class ChunkedSource extends AbstractSource {
    ...
    public long read(Buffer sink, long byteCount) throws IOException {
      ...
      if (bytesRemainingInChunk == 0 || bytesRemainingInChunk == NO_CHUNK_YET) {
        System.out.println("ChunkedSource url ");
        readChunkSize();
        if (!hasMoreChunks) return -1;
      }
