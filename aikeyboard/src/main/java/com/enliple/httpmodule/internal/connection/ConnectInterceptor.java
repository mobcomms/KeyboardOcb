...
public final class ConnectInterceptor implements Interceptor {
  public final MobonOkHttpClient client;

  public ConnectInterceptor(MobonOkHttpClient client) {
    this.client = client;
  }
  ...
  public MobonResponse intercept(Chain chain) throws IOException {
    ...
    // We need the network to satisfy this mobonRequest. Possibly for validating a conditional GET.
    boolean doExtensiveHealthChecks = !mobonRequest.method().equals("GET");
    HttpCodec httpCodec = streamAllocation.newStream(client, chain, doExtensiveHealthChecks);
    RealConnection connection = streamAllocation.connection();

    return realChain.proceed(mobonRequest, streamAllocation, httpCodec, connection);
  }
}
